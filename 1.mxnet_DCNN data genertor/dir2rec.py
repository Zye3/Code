...
def read_label(path_in):
  ...
  for line in open(path_in, 'r'):
    ...
    """
    item.bbox = np.array( [float(x) for x in line[137:]], dtype=np.float32 )
    """
    
    """
    item.landmark = np.array( [float(x) for x in line[1:137]], dtype=np.float32 ).reshape( (68, 2) )
    item.aligned = False
    item.id = _id
    item.label = label
    yield item
...
def image_encode(args, i, item, q_out):
    resut_path = "./new_dataset"
    if not os.path.isdir(resut_path):
    ...
    if item.flag==0:
      ...
      if item.aligned:
        ...
        # print(img, item, '%d,%d'%(args.image_h, args.image_w))
        img, dst_ = face_preprocess.preprocess(img, bbox = item.bbox, landmark= item.landmark, image_size='%d,%d'%(args.image_h, args.image_w))
        """
        20220508
        """
        save_imgpath = "{}/{}".format(resut_path, fullpath.split("/")[-1])
        cv2.imwrite(save_imgpath, img)
        save_ptspath = "{}/{}.pts".format(resut_path, fullpath.split("/")[-1].split(".")[0])
        points = ["version: 1\n", "n_points:  68\n", "{\n"]
        for mm in dst_:
            points.append(str(mm[0])+' '+str(mm[1])+"\n")
        points.append("}")
        with open(save_ptspath, "w") as f:
            for ii in points:
                f.write(ii)
    ...
    while more:
        ...
        while count in buf:
            ...
            if s is not None:
                ...
                # print(item)
                # record.write_idx(item[0], item[2].tostring())
...
def parse_args():
    ...
    parser.add_argument('--input', type=str, default="./Face_data", help='input data dir.')
    parser.add_argument('--output', type=str, default="20210516", help='outputdata dir.')
...
if __name__ == '__main__':
    args = parse_args()
    label_file = os.path.join(args.input, 'labels.txt')
