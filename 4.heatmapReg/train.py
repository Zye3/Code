...
def main(args):
  ...
  ctx = [mx.gpu(0)]
  args.ctx_num = len(ctx)# number of GPU

  args.batch_size = args.per_batch_size*args.ctx_num#batch_size = per_batch_size*number of gpu
  config.per_batch_size = args.per_batch_size
 


  print('Call with', args, config) #get args and config parameters
  ...
  initializer = mx.init.Xavier(rnd_type='gaussian', factor_type="in", magnitude=2) #initializer = Xavier
  ...
  def _batch_callback(param):
    ...
    for _lr in lr_steps:
      if mbatch==_lr: #change lr to lr *= 0.2
        opt.lr *= 0.2
...
if __name__ == '__main__':
  ...
  # general
  parser.add_argument('--network', help='network name', default=default.network, type=str) #loading default.network
  parser.add_argument('--dataset', help='dataset name', default=default.dataset, type=str) #loading default.dataset
  ...
  parser.add_argument('--per-batch-size', type=int, default=default.per_batch_size, help='') #loading default.per_batch_size
